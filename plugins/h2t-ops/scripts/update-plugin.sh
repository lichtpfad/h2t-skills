#!/usr/bin/env bash
# Update h2t-ops plugin in Claude Code cache.
# Copies repo-root lib/ into cache (packaging decision from spec).
# Usage: bash plugins/h2t-ops/scripts/update-plugin.sh [--push]
set -euo pipefail
...
CACHE_BASE="$HOME/.claude/plugins/cache/lichtpfad/h2t-ops"
...
# KEY: copy repo-root lib/ into cache
rm -rf "$CACHE_DIR/lib"
cp -r "$REPO_DIR/lib" "$CACHE_DIR/lib"

# Copy pyproject.toml so uv pip install works
...
LIB_OK=$([ -d "$CACHE_DIR/lib/clients" ] && echo "true" || echo "false")

...
- [ ] **subagent-driven-dev** — skill: `superpowers:subagent-driven-development` · input: `execute Tasks 1-8 of the plan in order; each session.py task re-syncs both vendored copies + parity + CI-scope green before commit` · done: all tasks green · failure: per-task gate; escalate on repeated fail · re-entry: continue from first unchecked task
- [ ] **gates** — skill: `codex + pre-merge-check` · input: `final codex review of the diff + council finish-gate (SOUND) + pre-merge-check` · done: no [P1]; suite green · failure: fix then re-run (<=N) · re-entry: idempotent: re-run gate
- [ ] **e2e** — skill: `real entrypoint run` · input: `N/A — local-only telemetry, no external service surface (push/central = #305)` · done: DONE / N/A / BLOCKED-DEFERRED · failure: BLOCKED->handoff; behavioral fail->fix · re-entry: idempotent: re-run
- [ ] **PR** — skill: `superpowers:finishing-a-development-branch` · input: `open PR from feat/skill-telemetry-l1-activation → main, link #289 children` · done: PR opened · failure: escalate · re-entry: continue: reuse branch

...
- [ ] **subagent-driven-dev** — skill: `superpowers:executing-plans (inline, no subagents)` · input: `Tasks 6-12 in order, codex review after each. 6 done (dc84c57)` · done: all tasks green · failure: per-task gate; escalate on repeated fail · re-entry: continue from first unchecked task
- [ ] **gates** — skill: `codex + pre-merge-check` · input: `cumulative review of the whole Wave 2 diff, then h2t-dev:pre-merge-check` · done: no [P1]; suite green · failure: fix then re-run (<=N) · re-entry: idempotent: re-run gate
- [ ] **e2e** — skill: `real entrypoint run` · input: `h2t-hook fires post-git-commit-docs-lint; wheel installed into a throwaway venv runs h2t-ops evals status` · done: DONE / N/A / BLOCKED-DEFERRED · failure: BLOCKED->handoff; behavioral fail->fix · re-entry: idempotent: re-run
- [ ] **PR** — skill: `superpowers:finishing-a-development-branch` · input: `one PR for Wave 2 off feat/wave2-consistency; #339 lands separately as its own PR` · done: PR opened · failure: escalate · re-entry: continue: reuse branch

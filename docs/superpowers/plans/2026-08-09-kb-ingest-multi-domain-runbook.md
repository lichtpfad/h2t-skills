...
- [ ] **subagent-driven-dev** — skill: `superpowers:executing-plans (inline TDD; 2 repos)` · input: `docs/superpowers/plans/2026-08-09-kb-ingest-multi-domain-plan.md` · done: E1 test green + skill edits applied · failure: per-task gate; escalate on repeated fail · re-entry: continue from first unchecked task
- [ ] **gates** — skill: `codex + pre-merge-check` · input: `engine E1 diff review + full suite; skill self-review` · done: no [P1]; suite green · failure: fix then re-run (<=N) · re-entry: idempotent: re-run gate

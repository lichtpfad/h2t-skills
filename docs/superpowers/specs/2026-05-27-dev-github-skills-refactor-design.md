...
| docs-sync-labels из lint? | Да — `docs-lint --fix-labels` запускает sync-labels. **Флаг `--fix` уже занят** (создаёт missing dirs, фиксирует frontmatter) — поэтому отдельный `--fix-labels`. |
